...
/**
 * @title Complete Local Deployment Script with Full SFlax Integration
 * @notice Comprehensive deployment script that includes all SFlax boost functionality
 * @dev Addresses all gaps identified in reflax-ui story 020
 */
contract CompleteLocalDeploymentScript is Script {
    struct DeploymentConfig {
        ...
        // Enhanced SFlax boost configuration
    ...
    function run() external {
        ...
        console.log("=== Starting Complete ReFlax Deployment with Full SFlax Integration ===");
        ...
        _configureSFlaxIntegration(deployed, config);
        _fundTestAccounts(deployed, config);
        _setupTestDeposits(deployed, config);
        _validateDeployment(deployed, config);
        _logDeploymentSummary(deployed, config);

        vm.stopBroadcast();
        console.log("=== Complete ReFlax Deployment with Full SFlax Integration Complete ===");
    }
    ...
    function _getDeploymentConfig() internal pure returns (DeploymentConfig memory) {
        return DeploymentConfig({
            ...
            testRewardsAmount: 10000 * 1e18  // 10,000 Flax rewards for testing
        });
    }
        ...
        returns (DeployedContracts memory) 
    {
        ...
        // Deploy Enhanced Vault with SFlax integration
        deployed.vault = address(new Vault(
            deployed.flax,
            deployed.sFlax,
            deployed.usdc,
        ...
        console.log("  Vault (Enhanced):", deployed.vault);
    ...
    function _configureContracts(
        ...
        // Whitelist vault in yield source
        CVX_CRV_YieldSource yieldSource = CVX_CRV_YieldSource(payable(deployed.yieldSource));
        yieldSource.whitelistVault(deployed.vault, true);
        console.log("  Whitelisted vault in yield source");
        
        // Fund contracts for swaps
        _fundContractsForSwaps(deployed, config);
        
        console.log("Basic contract configuration complete");
    }

    function _configureSFlaxIntegration(
        DeployedContracts memory deployed,
        DeploymentConfig memory config
    ) internal {
        console.log("Configuring enhanced SFlax integration...");
        
        Vault vault = Vault(payable(deployed.vault));
        MockSFlax sFlax = MockSFlax(deployed.sFlax);
        ...
        // Configure proper flaxPerSFlax exchange rate (already set in constructor, but verify)
        require(vault.flaxPerSFlax() == config.initialFlaxPerSFlax, "FlaxPerSFlax not properly initialized");
        console.log("  Verified flaxPerSFlax rate:", vault.flaxPerSFlax());
        
        // Configure sFlax burner permissions
        sFlax.setApprovedBurner(deployed.vault, true);
        console.log("  Approved vault as sFlax burner");
        
        // Fund vault with Flax for reward distribution
        MockERC20 flax = MockERC20(deployed.flax);
        flax.mint(deployed.vault, config.testRewardsAmount);
        console.log("  Funded vault with", config.testRewardsAmount, "Flax for rewards");
        
        console.log("SFlax integration configuration complete");
    }
    ...
    function _fundTestAccounts(
        ...
        fundingAmounts[0] = config.largeAccountFunding;
        fundingAmounts[1] = config.mediumAccountFunding; 
        fundingAmounts[2] = config.smallAccountFunding;
        fundingAmounts[3] = 0; // New user starts with no tokens
        ...
        for (uint256 i = 0; i < 4; i++) {
            ...
            if (amount > 0) {
                ...
                MockERC20(deployed.sFlax).mint(account, config.testSFlaxAmount); // Fixed sFlax for testing
                
                console.log("  Funded account", i + 1);
                console.log("    USDC amount:", amount);
                console.log("    sFlax amount:", config.testSFlaxAmount);
            }
        }
    }

    function _setupTestDeposits(
        DeployedContracts memory deployed,
        DeploymentConfig memory config
    ) internal {
        console.log("Setting up test deposits in vault...");
        
        Vault vault = Vault(payable(deployed.vault));
        MockERC20 usdc = MockERC20(deployed.usdc);
        
        uint256 largeDeposit = 5000 * 1e6;   // Large depositor: $5K into vault
        uint256 mediumDeposit = 500 * 1e6;   // Medium depositor: $500 into vault  
        uint256 smallDeposit = 50 * 1e6;     // Small depositor: $50 into vault
        
        uint256[3] memory depositAmounts;
        depositAmounts[0] = largeDeposit;
        depositAmounts[1] = mediumDeposit;
        depositAmounts[2] = smallDeposit;
        
        // Stop broadcasting to do test account setup
        vm.stopBroadcast();
        ...
        // Resume broadcasting for cleanup
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        vm.startBroadcast(deployerPrivateKey);
        
        console.log("Test deposit setup complete");
    }
    ...
    function _validateDeployment(
        ...
        // Validate boost calculations work
        uint256 testSFlaxAmount = 10 * 1e18; // 10 sFlax
        uint256 boostPercentage = vault.calculateBoostPercentage(testSFlaxAmount);
        require(boostPercentage > 0 && boostPercentage <= config.maxBoostPercentage, "Boost calculation failed");
        
        // Validate flaxPerSFlax is not fallback 1
        require(vault.flaxPerSFlax() != 1e18, "FlaxPerSFlax is fallback value");
        require(vault.flaxPerSFlax() == config.initialFlaxPerSFlax, "FlaxPerSFlax not configured properly");
        
        // Validate vault is approved burner
        require(sFlax.approvedBurners(deployed.vault), "Vault not approved as burner");
        
        // Validate test accounts have deposits
        require(vault.getEffectiveDeposit(deployed.testAccounts[0]) > 0, "Test accounts not setup with deposits");
        
        // Validate max boost percentage
        require(vault.getMaxBoostPercentage() == config.maxBoostPercentage, "Max boost percentage not set");
        
        // Validate optimal burn calculations work
        uint256 baseReward = 1000 * 1e18;
        uint256 optimalBurn = vault.getOptimalBurnAmount(baseReward);
        require(optimalBurn > 0, "Optimal burn calculation failed");
        
        console.log("All validations passed!");
        console.log("  Boost calculation:", boostPercentage, "bps for 10 sFlax");
        console.log("  FlaxPerSFlax rate:", vault.flaxPerSFlax());
        console.log("  Vault approved as burner:", sFlax.approvedBurners(deployed.vault));
        console.log("  Test deposit:", vault.getEffectiveDeposit(deployed.testAccounts[0]));
    }
    ...
    function _logDeploymentSummary(
        ...
        console.log("SFlax Integration Features:");
        console.log("  Max Boost Percentage:", vault.getMaxBoostPercentage(), "bps");
        console.log("  FlaxPerSFlax Rate:", vault.flaxPerSFlax(), "(not fallback 1)");
        console.log("  Vault Approved Burner:", MockSFlax(deployed.sFlax).approvedBurners(deployed.vault));
        console.log("");
        ...
        console.log("Test Accounts with Deposits:");
        for (uint256 i = 0; i < 3; i++) {
            uint256 deposit = vault.getEffectiveDeposit(deployed.testAccounts[i]);
            uint256 sFlaxBalance = MockSFlax(deployed.sFlax).balanceOf(deployed.testAccounts[i]);
            console.log("  Account", i + 1, ":", deployed.testAccounts[i]);
            console.log("    Vault Deposit:", deposit, "USDC");
            console.log("    sFlax Balance:", sFlaxBalance / 1e18, "sFlax");
        }
        console.log("");
        
        console.log("Story 001 Completion Status:");
        console.log("  [DONE] Enhanced deployment script created");
        console.log("  [DONE] Boost calculation methods implemented");
        console.log("  [DONE] flaxPerSFlax configured (not fallback 1)");
        console.log("  [DONE] SFlax burner permissions set");
        console.log("  [DONE] Test accounts with vault deposits");
        console.log("  [DONE] SFlax balances for boost testing");
        console.log("  [DONE] Boost algorithm with diminishing returns");
        console.log("  [DONE] Enhanced error handling");
        console.log("  [DONE] Gas estimation methods");
        console.log("");
        console.log("Ready for reflax-ui story 020 testing!");
    ...
    function _saveAddresses(DeployedContracts memory deployed) internal {
        ...
        json = string(abi.encodePacked(json, '  "enhancedFeatures": {\n'));
        json = string(abi.encodePacked(json, '    "sFlaxIntegration": true,\n'));
        json = string(abi.encodePacked(json, '    "boostCalculations": true,\n'));
        json = string(abi.encodePacked(json, '    "properFlaxPerSFlaxRate": true,\n'));
        json = string(abi.encodePacked(json, '    "testAccountDeposits": true\n'));
        ...
        console.log("Saved enhanced addresses to scripts/deployedAddresses.json");
    }
}